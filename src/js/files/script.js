...
const progress1 = Array.from(
  document.querySelectorAll(".main__slider-progress .progress")
);
...
const progress2 = Array.from(
  document.querySelectorAll(".features__slider-progress .progress")
);
...
progress1.forEach((el) =>
  el.addEventListener("animationend", slideShow1, false)
);
progress2.forEach((el) =>
  el.addEventListener("animationend", slideShow2, false)
);
...
function createEyeAnimation(canvasId, eyeRadiusValue = 50, variant = "big") {
  ...
  function init() {
    ...
    let eye1 = new Eye(canvas.width / 2.3, canvas.height / 2, eyeRadius);
    let eye2 = new Eye((3 * canvas.width) / 4, canvas.height / 2, eyeRadius);
...
function initProductCircleLoading() {
  let svgCircleProgressPaths = document.querySelectorAll(
    "[id^=svgCircleProgressPath]"
  );
  ...
  function animateBlocks(el, animateValue) {
    switch (el) {
      ...
      case "block2":
        ...
        setTransform(
          block2,
          `translate(${position_2}px, ${position_2 * 0.7}px)`
        );
        break;
      case "block3":
